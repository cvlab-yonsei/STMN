...
class STMN(nn.Module):
    def __init__(self, feat_dim=2048, num_class=710, stride=1):
        ...
        self.smem = mem.SpatialMemory(feat_dim=feat_dim, mem_size=args.smem_size, 
                                      margin=args.smem_margin)
        ...
        self.bn_s.bias.requires_grad_(False)  # no shift
        self.bn_s.apply(model.weights_init_kaiming)
        self.bn_t = nn.BatchNorm1d(feat_dim)
        self.bn_t.bias.requires_grad_(False)  # no shift
